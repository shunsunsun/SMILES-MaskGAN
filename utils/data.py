...
def remove_duplicate(list_with_duplicates):
    """
    Removes the duplicates and keeps the ordering of the orginal list.
...
def download_if_not_present(filename, url):
    """
    ...
    if os.path.isfile(filename):
        ...
        with open(filename, 'wb') as fd:
            print(f'Starting {filename} download from {url}...')
            
            with ProgressBarUpTo(unit='B', unit_scale=True, unit_divisor=1024, miniters=1) as t:
                urlretrieve(url, fd.name, reporthook=t.update_to)
