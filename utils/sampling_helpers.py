...
from utils.chemistry import is_valid, canonicalize


def sample_valid_molecules(model: DistributionMatchingGenerator, number_molecules: int, max_tries=10) -> List[str]:
    """
    Sample from the given generator until the desired number of valid moleculs
    ...
    Returns:
        A list of number_molecules valid molecules. If this was not possible with the given max_tries,
...
def sample_unique_molecules(model: DistributionMatchingGenerator, number_molecules: int, max_tries=10) -> List[str]:
    ...
    Returns:
        A list of number_molecules unique molecules, in canonicalized form.
