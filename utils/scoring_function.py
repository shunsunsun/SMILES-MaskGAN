...
from utils.chemistry import smiles_to_rdkit_mol
from utils.score_modifier import ScoreModifier, LinearModifier
from utils.math import geometric_mean
...
class ScoringFunction:
    ...
    In general, do not inherit directly from this class. Prefer 'MoleculewiseScoringFunction' or
...
class BatchScoringFunction(ScoringFunction):
    ...
    def score_list(self, smiles_list: List[str]) -> List[float]:
        ...
        scores = [self.corrupt_score if raw_scores is None
    ...
    def raw_score_list(self, smiles_list: List[str]) -> List[float]:
        ...
        Returns:
            A list of scores. For unsuccessful calculations or invalid molecules,
            'None' should be given as a value for the corresponding molecule.
        """
...
class ScoringFucntionWrapper(ScoringFunction):
    """
    ...
    def _increment_evaluation_count(self, n: int):
        ...
        However, adding a threading.Lock member variable makes the class non-pickle-able,
        which prevents any multithreading.
        Therefore, in the current implementation there cannot be a guarantee that
        self.evaluations will be calculated correctly.
        """
