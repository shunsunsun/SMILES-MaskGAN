...
from utils.descriptors import mol_weight, logP, num_H_donors, tpsa, num_atoms, AtomCounter
from utils.fingerprints import get_fingerprint
from utils.score_modifier import ScoreModifier, MinGaussianModifier, MaxGaussianModifier, GaussianModifier
from utils.scoring_function import ScoringFunctionBasedOnRdkitMol, MoleculewiseScoringFunction
from utils.chemistry import smiles_to_rdkit_mol, parse_molecular_formula
from utils.math import arithmetic_mean, geometric_mean
...
class IsomerScoringFunction(MoleculewiseScoringFunction):
    ...
    The score penalizes deviations from the required number of atoms for each element type,
    and for the total number of atoms.
    ...
    def __init__(self, molecular_formula: str, mean_function='geometric') -> None:
        ...
        self.scoring_function = self.determine_scoring_function(molecular_formula)
    ...
    @staticmethod
    def determine_scoring_function(molecular_formula: str) -> List[RdkitScoringFunction]:
    ...
    def raw_score(self, smiles: str) -> float:
        ...
        scores = [f.score(smiles) for f in self.scoring_function]
...
class SMARTSScoringFunction(ScoringFunctionBasedOnRdkitMol):
    """
    Test for SMARTS which should be or should not be present in the compound.
    """

    def __init__(self, target: str, inverse=False) -> None:
        """
        :param target: The SMARTS string to match.
        :param inverse: Specifies whether the SMARTS is desired (False) or an antipattern,
                        which we don't want to see in the molecule (inverse=False)
        """
