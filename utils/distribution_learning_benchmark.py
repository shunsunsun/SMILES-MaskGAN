...
from utils.chemistry import canonicalize_list, is_valid, calculate_pc_descriptors, continuous_kldiv, \
...
from utils.sampling_helpers import sample_valid_molecules, sample_unique_molecules
...
class ValidityBenchmark(DistributionLearningBenchmark):
    """
    Assess what percentage of molecules generated by a model are valid molecules (i.e., have a valid SMILES string)
    """
...
class UniquenessBenchmark(DistributionLearningBenchmark):
    """
    Assess what percentage of molecules generated by a model are unique.
    """
...
class NoveltyBenchmark(DistributionLearningBenchmark):
    ...
    def assess_model(self, model: DistributionMatchingGenerator) -> DistributionLearningBenchmarkResult:
        ...
        # canonicalize_list in order to remove stereo information
        # (also removes duplicates and invalid molecules, but ther shouldn't be any)
...
class KLDivBenchmark(DistributionLearningBenchmark):
    ...
    def __init__(self, number_samples: int, training_set: List[str]) -> None:
        ...
        self.training_set_molecules = canonicalize_list(get_random_subset(training_set, self.number_samples,
                                                                          seed=42), include_stereocenters=False)
    ...
    def assess_model(self, model: DistributionMatchingGenerator) -> DistributionLearningBenchmarkResult:
        ...
        # canonicalize_list in order to remove stereo information
        # (also removes duplicates and invalid molecules, but there shouldn't be any)
        unique_molecules = set(canonicalize_list(molecules, include_stereocenters=False))

        # first we calculate kl divergence for the float valued descriptors ...
