import sys
sys.path.append('C:/Users/youngjae/PycharmProjects/Chemistry')
...
from utils.data import remove_duplicate
...
def is_valid(smiles: str):
    ...
    Returns:
        True if the SMILE strings corresponds to a valid, non-empty molecule.
    """
...
def canonicalize_list(smiles_list: Iterable[str], include_stereocenters=True) -> List[str]:
    ...
    Args:
        smiles_list: moleculs as SMILES strings
        include_stereocenters: whether to keep the stereochemical information in teh canonical SMILES strings
    
    Returns:
    The canonicalized and filtered intput smiles.
    """
    ...
    return remove_duplicate(canonicalized_smiles)
...
def initialize_neutralization_reactions():
    patts = (
        # Imidazoles
        ('[n+;H]', 'n'),
        # Amines
...
def filter_and_canonicalize(smiles: str, holdout_set, holdout_fps, neutralization_rxns, 
                            tanimoto_cutoff=0.5, include_stereocenters=False):
    """
    Args:
    ...
    try:
        ...
        # balance charge if unbalanced
        if canon_smi.count('+') - canon_smi.count('-') != 0:
            new_mol, changed = neutralize_charges(mol, reactions=neutralization_rxns)
