...
class MaskGANModel(nn.Module):
    ...
    def sample(self, srcs, lengths, tgts, mask, vocab):

        with torch.no_grad():
            logits = self.generator.logits(srcs, lengths, tgts)

            samples = []
            for t in range(logits.size(1)):  # sequence length
                logit = logits[:, t, :]
                sampled = logit.argmax()
            ...
            smiles = self.id2string(ids, vocab, rem_eos=True)

        return smiles        
    
    def id2string(self, ids, vocab, rem_eos=True):

        if rem_eos and ids[-1] == vocab['<eos>']:
            ids = ids[:-1]
        ...
        string = ''.join([sample for sample in samples])
